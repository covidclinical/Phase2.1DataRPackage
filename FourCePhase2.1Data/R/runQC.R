...
runQC <- function(siteid){
  ...
  for(ii in 1:length(phase1.1.res)){
    ...
    tmp=phase1.1.res[[nm]]
    if(dim(tmp$err.report)[1]!=0 & any(is.na(tmp$err.report)!=1)){print("Error found.")}else{print("Ok ...")}
  }
  
  # QC for Phase2.1
  file.nm2=paste0(dir.output, "/Phase2.1DataQCReport.", siteid,".doc")
  rtffile <- RTF(file.nm2)  
  runQC_Phase2.1_report(rtffile,phase2.ClinicalCourse, phase2.PatientObservations, phase2.PatientSummary, phase1.DailyCounts, phase1.ClinicalCourse, phase1.Demographics,phase1.Diagnoses, phase1.Labs, phase1.Medications, output.dir, site.nm=siteid)
